...
export default function AdoptionReviewBoard() {
  ...
  const adoptionReview = useSelector(
    (state) => state.adoptionReview.adoptionReview
  );
  ...
  useEffect(() => {
    try {
      axios
        .get(`https://i9a203.p.ssafy.io/backapi/api/v1/board`, {
          option: "all",
        })
        .then((response) => {
  ...
  return (
    ...
    >
      ...
      <Grid container spacing={1}>
        {adoptionReview.length > 0 ? ( // 입양후기가 있을 때에만 페이지네이션 표시
          <>
            {/* 입양후기 리스트 */}
            <Grid
              container
